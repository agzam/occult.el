...
Set ~occult-summary-end-regexp~ where lines end with a marker that does not belong in the summary - a status glyph, an elapsed time. The variable is buffer-local when set. The summary stops at the start of the first match on its line, even when ~occult-summary-max-length~ would cut inside the match, while the buffer text keeps the marker. Whatever follows the match on that line is hidden with it: the option ends the summary early, it does not cut the match out of the middle. A match at the very start is skipped, so a fold always shows at least one character.
